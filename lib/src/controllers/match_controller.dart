...
final class MatchController {
  MatchController(
    ...
    Map<String, dynamic>? params,
            ...
            findPathFromName(sPath, params ?? <String, dynamic>{})),
  ...
  factory MatchController.fromName(
      String name, String foundPath, OrioleRouteChildren routes,
      {Map<String, dynamic>? params}) {
  ...
  Future<OrioleRouteInternal> get match async {
    // Check if this route should be mocked
    final result = await _searchMatch();
    Oriole.to.params.updateParams(result.getAllParams());
    return result;
  }

  Future<OrioleRouteInternal?> _addInitRouterIfNeeded(
      OrioleRouteInternal route) async {
    // There is no init route
    if (!route.route.withChildRouter) return null;
    // navigator not created, don't add init route
    if (!Oriole.to.hasNavigator(route.fullPath)) return null;
    ...
    if (routeSegment.startsWith('/:')) {
      ...
      if (name.contains('(') && name.contains(')')) {
        try {
          ...
          Oriole.log('Error finding regex match for $name: $e');
          return false;
        }
      }
      if (!params.asMap.containsKey(name)) {
        ...
        Oriole.log('Duplicate param name $name');
      }
      return true;
    }
    return false;
  ...
  static String findPathFromName(String name, Map<String, dynamic> params) {
    ...
    for (var param in Oriole.to.params.asMap.entries) {
      if (newPath.contains(':${param.key}')) {
        newPath =
            newPath.replaceAll(':${param.key}', _getParamString(param.value));
      }
  ...
  Future<OrioleRouteInternal> _searchMatch() async {
    var searchIn = routes;
    if (path.pathSegments.isEmpty) {
      final match = await _tryFind(searchIn, -1);
      if (match == null) {
        return OrioleRouteInternal.notFound(parentPath + path.toString());
      }
  ...
  Future<OrioleRouteInternal?> _tryFind(OrioleRouteChildren routes, int index,
      {bool updatePath = true}) async {
    final path = index == -1 ? '' : this.path.pathSegments[index];
    bool isSamePath(OrioleRouteInternal route) {
      return route.route.path == '/$path';
    }

    bool isComponent(OrioleRouteInternal route) {
    ...
    bool findMulti(OrioleRouteInternal route) {
    ...
    bool findNoPath(OrioleRouteInternal route) => route.route.path == '/!';
    ...
    /// see #111
    final isRoot =
        routes.parentKey.name == OrioleNavigatorContext.rootRouterName;
    ...
    Oriole.log('[$path] is not child of ${routes.parentKey}');
    return null;
  }
